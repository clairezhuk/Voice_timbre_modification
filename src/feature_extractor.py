...
import librosa
import numpy as np
from rmvpe.src.inference import RMVPE

class FeatureExtractor:
    def __init__(self, hubert_path, rmvpe_path, device="cuda"):
        self.device = device
        self.hubert = self._load_hubert(hubert_path)
        # Оновлена ініціалізація RMVPE
        self.rmvpe = RMVPE(rmvpe_path, device=self.device)

    def _load_hubert(self, path):
        # HuBERT потребує fairseq для завантаження [cite: 114]
    ...
    def extract_features(self, audio_path):
        ...
        # Звільнення пам'яті перед обробкою [cite: 229]
        ...
        # 2. Екстракція F0 через RMVPE (найкраще для 2 ГБ) [cite: 203]
        f0 = self.rmvpe.infer_from_audio(audio, sample_rate=16000)
